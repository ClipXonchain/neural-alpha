...
function log(
  ...
  const prefix = {
    ...
    brain: "\x1b[96m[BRAIN]\x1b[0m",
  }[level];

  const txSuffix = txHash ? ` tx:${txHash}` : "";
  const line = narrative ?? event;
  console.log(`${prefix} ${entry.timestamp} ${line}${txSuffix}`, data ? JSON.stringify(data) : "");
...
export const logger = {
  ...
  trade: (event: string, data?: Record<string, unknown>, txHash?: string) =>
    log("trade", event, data, txHash),
  ...
  brain: (narrative: string, data?: Record<string, unknown>, txHash?: string) =>
    log("brain", narrative, data, txHash, narrative),
};
